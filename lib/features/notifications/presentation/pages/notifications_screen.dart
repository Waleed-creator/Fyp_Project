...
import '../../domain/entities/notification_entity.dart'; // ← NotificationEntity
import '../../domain/entities/alert_type.dart'; // ← AlertType
// import 'recommendations_tab.dart';

// ✅ No enum or model class needed here — both come from domain layer
...
class _NotificationsScreenState extends State<NotificationsScreen>
  ...
  Future<void> _loadParentId() async {
    ...
    if (currentUser != null) {
      ...
      debugPrint(
        '📡 [NotificationScreen] Parent ID loaded from Firebase Auth: $parentId',
      );
      setState(() {
        _parentId = parentId;
      });
      ...
      debugPrint('❌ [NotificationScreen] User not logged in');
      _auth.authStateChanges().listen((User? user) {
        if (user != null && mounted) {
          debugPrint('📡 [NotificationScreen] User logged in: ${user.uid}');
          setState(() {
            _parentId = user.uid;
          });
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('$deletedCount notification(s) deleted'),
      ...
      body: _parentId == null
          ...
          : TabBarView(
              ...
              children: [
                ...
                // RecommendationsTab(parentId: _parentId!),
              ],
            ),
    );
  }
  ...
  Widget _buildNotificationsTab() {
    return BlocBuilder<NotificationBloc, NotificationState>(
      builder: (context, state) {
        debugPrint(
          '📡 [NotificationScreen] Current state: ${state.runtimeType}',
        );
        ...
        if (state is NotificationError) {
          debugPrint('❌ [NotificationScreen] Error state: ${state.message}');
          return Center(
            child: Column(
                ...
                return _buildNotificationCard(
                  ...
                  onTap: () {
                    if (_isSelectionMode) {
                      ...
                      if (actionUrl != null) {
                        if (actionUrl == '/flagged-calls' ||
                            actionUrl == '/calls/suspicious' ||
                            actionUrl.contains('flagged') ||
                            actionUrl.contains('suspicious')) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text(
                                'Flagged calls feature has been removed',
                              ),
                            ),
                          );
                        }
  ...
  Widget _buildNotificationCard({
    ...
    return Card(
      ...
      color: isSelected
          ? AppColors.darkCyan.withValues(alpha: 0.1)
  ...
  IconData _getIconForAlertType(AlertType alertType) {
    switch (alertType) {
  ...
  Color _getColorForAlertType(AlertType alertType) {
    switch (alertType) {
      case AlertType.sos:
        return Colors.red;
      case AlertType.suspiciousMessage:
