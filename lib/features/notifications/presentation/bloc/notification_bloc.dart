...
class NotificationBloc extends Bloc<NotificationEvent, NotificationState> {
  ...
  Future<void> _onStreamNotifications(
    ...
    debugPrint(
      '📡 [NotificationBloc] Starting stream for parent: ${event.parentId}',
    );
    emit(NotificationLoading());
    ...
    await emit.forEach(
      ...
      onData: (result) {
        return result.fold(
          (failure) {
            debugPrint('❌ [NotificationBloc] Stream error: ${failure.message}');
            return NotificationError(message: failure.message);
          },
          (notifications) {
            debugPrint(
              '✅ [NotificationBloc] Received ${notifications.length} notifications',
            );
      ...
      onError: (error, stackTrace) {
        debugPrint('❌ [NotificationBloc] Stream onError: $error');
        debugPrint('❌ [NotificationBloc] Stack trace: $stackTrace');
