...
class NotificationRepositoryImpl implements NotificationRepository {
  ...
  Stream<Either<Failure, List<NotificationEntity>>> streamNotifications(
    ...
    try {
      debugPrint(
        '📡 [NotificationRepo] Starting stream for parent: $parentId, child: $childId',
      );
      return remoteDataSource
          .streamNotifications(parentId, childId: childId)
          .map<Either<Failure, List<NotificationEntity>>>((notifications) {
            debugPrint(
              '📡 [NotificationRepo] Received ${notifications.length} notifications',
            );
            return Right<Failure, List<NotificationEntity>>(notifications);
          })
          .handleError((error, stackTrace) {
            debugPrint('❌ [NotificationRepo] Stream error: $error');
            debugPrint('❌ [NotificationRepo] Stack trace: $stackTrace');
            return Left<Failure, List<NotificationEntity>>(
              ServerFailure(error.toString()),
            );
          });
    } catch (e) {
      debugPrint('❌ [NotificationRepo] Exception in streamNotifications: $e');
      return Stream.value(
