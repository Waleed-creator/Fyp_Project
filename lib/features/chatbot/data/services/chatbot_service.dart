...
class ChatbotService {
  ...
  Future<ChatbotResponse> processChat({
    ...
    try {
      debugPrint('💬 [ChatbotService] Processing chat: "$userMessage"');
      ...
      if (childName != null) {
        childId = await _firebaseService.findChildIdByName(
          childName,
          parentId: parentId,
        );
      } else {
      ...
      if (childId != null) {
        childData = await _firebaseService.getFullChildData(
          childId,
          parentId: parentId,
        );
        debugPrint('✅ [ChatbotService] Found child data for: $childId');
      } else {
        debugPrint(
          '⚠️ [ChatbotService] No child found, proceeding without child data',
        );
      }

      // Step 3: Get AI response
      debugPrint('🤖 [ChatbotService] Getting AI response...');
      ...
      debugPrint('❌ [ChatbotService] Error processing chat: $e');
      ...
      return ChatbotResponse(message: errorMessage, error: e.toString());
    }
  }
  ...
  Stream<ChatbotResponse> processChatStream({
    ...
    try {
      ...
      debugPrint('❌ [ChatbotService] Error in stream: $e');
      debugPrint('❌ [ChatbotService] Stack trace: $stackTrace');
      ...
      yield ChatbotResponse(message: errorMessage, error: e.toString());
    }
  }
  ...
  Future<void> _saveChatHistory({
    ...
    try {
      ...
      debugPrint('✅ [ChatbotService] Chat history saved');
    } catch (e) {
      debugPrint('⚠️ [ChatbotService] Error saving chat history: $e');
    }
  }
