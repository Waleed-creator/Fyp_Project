...
class FirebaseChildDataService {
  ...
  Future<String?> findChildIdByName(String name, {required String parentId}) async {
    try {
      debugPrint('🔍 [FirebaseChildDataService] Searching for child: "$name" for parent: $parentId');
      ...
      if (allChildren.docs.isEmpty) {
        debugPrint('⚠️ [FirebaseChildDataService] No children found for parent: $parentId');
        return null;
      }
      ...
      for (var doc in allChildren.docs) {
        ...
        if (fullName.isNotEmpty && (fullName == searchName || fullName.contains(searchName))) {
          debugPrint('✅ [FirebaseChildDataService] Found by name field: ${doc.id}');
          return doc.id;
        }
        ...
        if (isSingleWord) {
          ...
          if (firstName.isNotEmpty && firstName == searchName) {
            debugPrint('✅ [FirebaseChildDataService] Found by firstName (exact): ${doc.id}');
            return doc.id;
          }
          
          // Then check lastName
          if (lastName.isNotEmpty && lastName == searchName) {
            debugPrint('✅ [FirebaseChildDataService] Found by lastName (exact): ${doc.id}');
            return doc.id;
          }
          
          // Then check if searchName is part of firstName
          if (firstName.isNotEmpty && firstName.contains(searchName)) {
            debugPrint('✅ [FirebaseChildDataService] Found by firstName (partial): ${doc.id}');
            return doc.id;
          }
          
          // Then check if searchName is part of lastName
          if (lastName.isNotEmpty && lastName.contains(searchName)) {
            debugPrint('✅ [FirebaseChildDataService] Found by lastName (partial): ${doc.id}');
            return doc.id;
          }
        } else {
          ...
          if (firstName.isNotEmpty && lastName.isNotEmpty) {
            ...
            if (combinedName == searchName || combinedName.contains(searchName)) {
              debugPrint('✅ [FirebaseChildDataService] Found by combined name: ${doc.id}');
              return doc.id;
            }
          }
          ...
          if (firstName.isNotEmpty && (firstName == searchName || firstName.contains(searchName))) {
            debugPrint('✅ [FirebaseChildDataService] Found by firstName: ${doc.id}');
            return doc.id;
          }
          
          // Check lastName match
          if (lastName.isNotEmpty && (lastName == searchName || lastName.contains(searchName))) {
            debugPrint('✅ [FirebaseChildDataService] Found by lastName: ${doc.id}');
            return doc.id;
          }
        }
        ...
        if (firstName.isNotEmpty && firstName.contains(searchName)) {
          debugPrint('✅ [FirebaseChildDataService] Found by firstName (fallback): ${doc.id}');
          return doc.id;
        }
        
        if (lastName.isNotEmpty && lastName.contains(searchName)) {
          debugPrint('✅ [FirebaseChildDataService] Found by lastName (fallback): ${doc.id}');
          return doc.id;
        }
      }

      debugPrint('⚠️ [FirebaseChildDataService] No child found with name: "$name"');
      return null;
    } catch (e) {
      debugPrint('❌ [FirebaseChildDataService] Error finding child: $e');
      return null;
    }
  }
  ...
  Future<Map<String, dynamic>?> getFullChildData(String childId, {required String parentId}) async {
    try {
      debugPrint('📊 [FirebaseChildDataService] Fetching full data for child: $childId');
      ...
      final data = {
        "profile": childProfile.data() ?? <String, dynamic>{},
        "appUsage": appUsage,
      ...
      debugPrint('✅ [FirebaseChildDataService] Successfully fetched child data');
      return data;
    } catch (e) {
      debugPrint('❌ [FirebaseChildDataService] Error loading full child data: $e');
      return null;
    }
  }
  ...
  Future<List<Map<String, dynamic>>> _getCollectionData(
    ...
    try {
      ...
      debugPrint('⚠️ [FirebaseChildDataService] Error fetching $collectionName: $e');
      return [];
    }
  }
