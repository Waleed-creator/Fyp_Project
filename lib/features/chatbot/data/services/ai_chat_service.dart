...
class AIChatService {
  ...
  void initialize({String? apiKey}) {
    if (_initialized && _apiKey != null) {
      debugPrint('⚠️ [AIChatService] Already initialized');
      return;
    }
    ...
    if (key.isEmpty || key == 'YOUR_OPENAI_API_KEY_HERE') {
      debugPrint('❌ [AIChatService] API key not configured. Please add it in chatbot_api_config.dart');
      _initialized = false;
      return;
    }
    ...
    try {
      ...
      if (!key.startsWith('sk-')) {
        debugPrint('❌ [AIChatService] Invalid API key format. OpenAI keys start with "sk-"');
        _initialized = false;
        return;
      }
      ...
      _initialized = true;
      debugPrint('✅ [AIChatService] Initialized successfully with OpenAI');
      debugPrint('✅ [AIChatService] Model: $_modelName');
      debugPrint('✅ [AIChatService] API key: ${key.substring(0, 10)}...');
    } catch (e, stackTrace) {
      debugPrint('❌ [AIChatService] Error initializing: $e');
      debugPrint('❌ [AIChatService] Stack trace: $stackTrace');
      _initialized = false;
    }
  }
  ...
  Future<String> askAI({
    ...
    try {
      debugPrint('🤖 [AIChatService] Generating response with OpenAI...');
...
""";
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint('✅ [AIChatService] Response generated');
        return text;
      } else {
        final errorData = jsonDecode(response.body);
        final errorMsg = errorData['error']?['message'] ?? 'Unknown error';
        debugPrint('❌ [AIChatService] OpenAI API error: $errorMsg');
...
""";
      ...
      debugPrint('❌ [AIChatService] Error streaming response: $e');
      debugPrint('❌ [AIChatService] Stack trace: $stackTrace');
