...
class _ChatbotScreenState extends State<ChatbotScreen> {
  ...
  Future<void> _initializeChatbot() async {
    if (!ChatbotApiConfig.isApiKeyConfigured) {
      setState(() {
        _messages.add(
          ChatMessage(
            text:
                '⚠️ API key not configured. Please add your OpenAI API key in:\nlib/features/chatbot/data/config/chatbot_api_config.dart',
            isUser: false,
    ...
    setState(() {
      ...
      _messages.add(
        ChatMessage(
          text:
              '👋 Hello! I\'m SafeNest AI Assistant. I can help you understand your children\'s digital activity, screen time, app usage, and more. How can I assist you today?',
          isUser: false,
          timestamp: DateTime.now(),
        ),
      );
  ...
  Future<void> _sendMessage() async {
    ...
    try {
      ...
      if (!_isInitialized) {
        setState(() {
          _messages.add(
            ChatMessage(
              text:
                  '⚠️ AI service is not initialized. Please check API key configuration.',
              isUser: false,
              timestamp: DateTime.now(),
            ),
          );
      ...
      debugPrint('❌ [ChatbotScreen] Error in _sendMessage: $e');
                    ...
                    child: Column(
                      ...
                      children: [
                        Icon(
                          Icons.smart_toy,
                          size: 64,
                          color: AppColors.darkCyan.withValues(alpha: 0.5),
                        ),
