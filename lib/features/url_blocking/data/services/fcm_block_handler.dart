...
class FcmBlockHandler {
  ...
  void initializeFcmListener() {
    ...
    debugPrint('✅ FCM block handler initialized');
  }
  ...
  void _handleBlockUpdateMessage(RemoteMessage message) {
    ...
    if (type == 'blockRulesUpdated') {
      debugPrint('🔔 FCM: Block rules updated, refreshing...');
  ...
  Future<void> _refreshBlockRules() async {
    try {
      ...
      debugPrint('✅ Block rules refreshed via FCM');
    } catch (e) {
      debugPrint('❌ Error refreshing block rules: $e');
    }
  }
}
