...
class ChildVpnBlockingService {
  ...
  final UrlBlockingFirebaseService _blockingService =
      UrlBlockingFirebaseService();
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;
  final FirebaseAuth _auth = FirebaseAuth.instance;
  final UrlTrackingFirebaseService _urlTrackingService =
      UrlTrackingFirebaseService();
  ...
  Future<void> initialize() async {
    if (_isInitialized) {
      debugPrint('⚠️ [VPNBlocking] Already initialized');
      return;
    }
    ...
    try {
      ...
      if (currentUser == null) {
        debugPrint('⚠️ [VPNBlocking] No user logged in, cannot initialize');
        return;
      }
      ...
      if (_parentId == null) {
        debugPrint('⚠️ [VPNBlocking] Parent ID not found for child $_childId');
        return;
      }

      debugPrint('🚀 [VPNBlocking] Initializing VPN blocking service...');
      ...
      if (!vpnStarted) {
        debugPrint('❌ [VPNBlocking] Failed to start VPN service');
        return;
      }
      ...
      _isInitialized = true;
      debugPrint('✅ [VPNBlocking] VPN blocking service initialized');
    } catch (e) {
      debugPrint('❌ [VPNBlocking] Error initializing: $e');
    }
  }
  ...
  Future<void> _handleMethodCall(MethodCall call) async {
    try {
      ...
      debugPrint('❌ [VPNBlocking] Error handling method call: $e');
    }
  }
  ...
  Future<bool> _startVpn() async {
    try {
      ...
      if (hasPermission != true) {
        debugPrint('⚠️ [VPNBlocking] VPN permission not granted');
        return false;
      }
      ...
      _isVpnRunning = true;
      debugPrint('✅ [VPNBlocking] VPN service started');
      return true;
    } on PlatformException catch (e) {
      debugPrint('❌ [VPNBlocking] Error starting VPN: ${e.message}');
      return false;
    }
  }
  ...
  Future<void> _loadAndUpdateBlockedUrls() async {
    try {
      ...
      debugPrint('📋 [VPNBlocking] Loaded ${domains.length} blocked domains');

      // Update VPN with blocked domains
      await _updateVpnBlockedDomains(domains);
    } catch (e) {
      debugPrint('❌ [VPNBlocking] Error loading blocked URLs: $e');
    }
  }
  ...
  Future<void> _updateVpnBlockedDomains(List<String> domains) async {
    try {
      if (!_isVpnRunning) {
        debugPrint('⚠️ [VPNBlocking] VPN not running, cannot update domains');
        return;
      }

      await _channel.invokeMethod('updateBlockedUrls', {'urls': domains});
      debugPrint(
        '✅ [VPNBlocking] Updated VPN with ${domains.length} blocked domains',
      );
    } on PlatformException catch (e) {
      debugPrint('❌ [VPNBlocking] Error updating VPN domains: ${e.message}');
    }
  }
  ...
  void _startListening() {
    ...
    _blockRulesSubscription = _blockingService
        ...
        .listen(
          (blockedUrls) async {
            debugPrint(
              '🔄 [VPNBlocking] Block rules updated: ${blockedUrls.length} rules active',
            );
          ...
          onError: (error) {
            debugPrint('❌ [VPNBlocking] Error in block rules stream: $error');
          },
        );
  }
  ...
  Future<void> logVisitedUrl(String url) async {
    try {
      ...
      await _urlTrackingService.uploadUrlToFirebase(
        ...
        metadata: {'source': 'vpn_dns_blocking', 'domain': domain},
      );

      debugPrint('📝 [VPNBlocking] Logged visited URL: $url');
    } catch (e) {
      debugPrint('❌ [VPNBlocking] Error logging visited URL: $e');
    }
  }
  ...
  Future<String?> _getParentId(String childId) async {
    try {
      ...
      debugPrint('❌ [VPNBlocking] Error getting parent ID: $e');
      return null;
    }
  }
  ...
  Future<void> stopVpn() async {
    try {
      ...
      _isVpnRunning = false;
      debugPrint('🛑 [VPNBlocking] VPN service stopped');
    } on PlatformException catch (e) {
      debugPrint('❌ [VPNBlocking] Error stopping VPN: ${e.message}');
    }
  }
