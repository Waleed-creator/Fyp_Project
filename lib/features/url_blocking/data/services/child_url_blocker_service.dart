...
class ChildUrlBlockerService {
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      if (currentUser == null) {
        debugPrint('⚠️ No user logged in, cannot initialize URL blocker');
        return;
      }
      ...
      if (parentId == null) {
        debugPrint('⚠️ Parent ID not found for child $childId');
        return;
      }
      ...
      _isInitialized = true;
      debugPrint('✅ URL Blocker Service initialized');
    } catch (e) {
      debugPrint('❌ Error initializing URL blocker: $e');
    }
  }
  ...
  Future<void> _loadBlockedUrls({
    ...
    try {
      ...
      debugPrint('✅ Loaded ${_blockedUrls.length} blocked URL rules');
    } catch (e) {
      debugPrint('❌ Error loading blocked URLs: $e');
    }
  }

  /// Start listening for real-time block rule updates
  void _startListening({required String childId, required String parentId}) {
    ...
    _blockRulesSubscription = _blockingService
        ...
        .listen(
          (blockedUrls) {
            ...
            debugPrint(
              '🔄 Block rules updated: ${_blockedUrls.length} rules active',
            );
          },
          onError: (error) {
            debugPrint('❌ Error in block rules stream: $error');
          },
        );
        ...
        .listen(
          (snapshot) {
            if (snapshot.docs.isNotEmpty) {
              debugPrint('🔔 Block rules notification received, refreshing...');
