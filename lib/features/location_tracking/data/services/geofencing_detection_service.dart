...
class GeofencingDetectionService {
  ...
  Future<void> startGeofencingMonitoring() async {
    if (_isMonitoring) {
      debugPrint('⚠️ Geofencing monitoring already active');
      return;
    }
    ...
    try {
      ...
      if (_parentId == null || _childId == null) {
        debugPrint('❌ Parent or child ID not found');
        return;
      }

      debugPrint('🚀 Starting geofencing monitoring for child: $_childId');
      ...
      _geofenceStream = _geofenceDataSource
          .streamGeofenceZones(_childId!)
          .listen(
            (geofences) {
              _activeGeofences = geofences.where((g) => g.isActive).toList();
              debugPrint(
                '📍 Active geofences updated: ${_activeGeofences.length}',
              );
            ...
            onError: (error) =>
                debugPrint('❌ Error streaming geofences: $error'),
          );
      ...
      debugPrint('✅ Geofencing monitoring started');
    } catch (e) {
  ...
  Future<void> stopGeofencingMonitoring() async {
    ...
    debugPrint('🛑 Stopping geofencing monitoring...');
    _isMonitoring = false;
    ...
    debugPrint('✅ Geofencing monitoring stopped');
  }
    ...
    try {
      for (var zone in _activeGeofences) {
        ...
        if (!wasInside && isInside) {
          debugPrint('🟢 Child entered zone: ${zone.name}');
          await _handleZoneEntry(zone, position);
        }
        // Detect exit
        else if (wasInside && !isInside) {
          debugPrint('🔴 Child exited zone: ${zone.name}');
  ...
  Future<void> _handleZoneEntry(
    ...
    try {
      ...
      debugPrint('✅ Zone entry event saved');
      ...
      debugPrint('✅ Entry notification sent to parent');
    } catch (e) {
      debugPrint('❌ Error handling zone entry: $e');
    }
  }
  ...
  Future<void> _handleZoneExit(
    ...
    try {
      ...
      // Save to Firestore
      await _geofenceDataSource.createZoneEvent(event);
      debugPrint('✅ Zone exit event saved');
      ...
      debugPrint('✅ Exit notification sent to parent');
    } catch (e) {
      debugPrint('❌ Error handling zone exit: $e');
    }
  }
