...
class _ParentQRScreenState extends State<ParentQRScreen> {
  ...
  Timer? _updateTimer; // Timer to update UI every second
  ...
  Future<void> _generateQRCode() async {
    try {
      ...
      if (widget.childData != null) {
        ...
        debugPrint('🔍 [ParentQR] Generated QR with child data embedded');
      } else {
        // Fallback to simple parent UID if no child data
        qrDataString = parentUid;
        debugPrint('🔍 [ParentQR] Using parent UID as QR data: $parentUid');
      }
      
      debugPrint('🔍 [ParentQR] QR code generated at: $now');
      ...
      try {
        ...
        debugPrint('⚠️ [ParentQR] Error storing QR timestamp: $e');
      ...
      debugPrint('🔍 [ParentQR] State updated - _qrData: $_qrData');

      // Start expiration timer (5 minutes)
      _startExpirationTimer();
      // Start UI update timer (updates every second)
      _startUpdateTimer();
      ...
      debugPrint('❌ [ParentQR] Error generating QR code: $e');
      setState(() {
  ...
  void _startUpdateTimer() {
    ...
    _updateTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
      ...
      setState(() {
        // Just trigger rebuild to update timer display
      });
    });
  }
  ...
  Future<void> _checkQRCodeExpiration() async {
    try {
        ...
        if (expiresAt != null) {
          ...
          if (now.isAfter(expirationTime)) {
            ...
            if (generatedAt != null) {
              ...
              // If no timestamp exists, set current time (for new QR codes)
              setState(() {
                _qrCodeGeneratedAt = DateTime.now();
                _isExpired = false;
              });
              _startExpirationTimer();
              _startUpdateTimer();
          ...
          // No expiration set, but QR data exists - set default expiration
          setState(() {
            _qrCodeGeneratedAt = DateTime.now();
            _isExpired = false;
          });
      ...
      debugPrint('⚠️ [ParentQR] Error checking expiration: $e');
    }
  }
  ...
  void _setupChildListener() {
    ...
    if (currentUser != null) {
      ...
      _childrenStream = FirebaseFirestore.instance
        ...
        debugPrint('🔄 [ParentQR] Children count changed: ${snapshot.docs.length}');
  ...
  String _getTimeUntilExpiration() {
    if (_qrCodeGeneratedAt == null) {
      // If no timestamp, assume 5 minutes from now
      return '5:00';
    }
  ...
  Widget build(BuildContext context) {
    ...
    debugPrint('🔍 [ParentQR] Build called - _isLoading: $_isLoading, _error: $_error, _qrData: $_qrData');
            ...
            children: [
              ...
              if (_isLoading)
                ...
                Column(
                  ...
                  children: [
                    // Always show timer if QR data exists
                    Container(
                        padding: EdgeInsets.symmetric(
