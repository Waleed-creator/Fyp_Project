...
class _ChildPermissionsScreenState extends State<ChildPermissionsScreen> with WidgetsBindingObserver {
  ...
  Future<void> _handleContinue() async {
    try {
      ...
      debugPrint('🔐 REQUESTING PERMISSIONS: Starting permission requests...');
      
      for (final entry in _permissionsStatus.entries) {
        if (entry.value) {
          debugPrint('   Requesting ${entry.key}: ${entry.value}');
          final status = await entry.key.request();
          debugPrint('   Result: ${status.isGranted ? "GRANTED" : "DENIED"}');
        }
      }
      ...
      debugPrint('   SMS: ${smsGranted ? "GRANTED" : "DENIED"}');
      debugPrint('   Phone: ${phoneGranted ? "GRANTED" : "DENIED"}');
      debugPrint('   Accessibility: ${accessibilityGranted ? "GRANTED" : "DENIED"}');
      debugPrint('   Usage Access: ${usageAccessGranted ? "GRANTED" : "DENIED"}');
      debugPrint('   Battery: ${batteryGranted ? "GRANTED" : "DENIED"}');

      if (smsGranted && phoneGranted && accessibilityGranted && usageAccessGranted) {
        debugPrint('✅ PERMISSIONS GRANTED: Starting message monitoring initialization...');
        ...
        debugPrint('⚠️ PERMISSIONS PARTIAL: Some permissions not granted');
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
  ...
  Future<void> _initializeMessageMonitoring() async {
    try {
      debugPrint('🚀 INITIALIZING TRACKING SERVICES: Starting after permissions...');
      
      // Start URL tracking
      final urlTrackingStarted = await ChildPermissionService.startUrlTracking();
      debugPrint('   URL Tracking: ${urlTrackingStarted ? "STARTED" : "FAILED"}');
      
      // Start app usage tracking
      final appTrackingStarted = await ChildPermissionService.startAppUsageTracking();
      debugPrint('   App Usage Tracking: ${appTrackingStarted ? "STARTED" : "FAILED"}');
      
      debugPrint('✅ TRACKING SERVICES: Initialized successfully');
    } catch (e) {
      debugPrint('❌ TRACKING SERVICES ERROR: $e');
    }
  }
  ...
  Future<void> _syncInstalledAppsAfterPermissions() async {
    try {
      debugPrint('🔄 [ChildPermissions] ========== AUTO-SYNCING INSTALLED APPS ==========');
      ...
      if (parentId == null || childId == null) {
        debugPrint('❌ [ChildPermissions] Cannot sync: parentId or childId is null');
        debugPrint('   parentId: $parentId, childId: $childId');
        return;
      }
      ...
      debugPrint('✅ [ChildPermissions] Installed apps auto-sync initiated');
      debugPrint('📱 [ChildPermissions] Apps will sync to Firebase automatically');
      debugPrint('🔄 [ChildPermissions] ================================================');
    } catch (e, stackTrace) {
      debugPrint('❌ [ChildPermissions] Error auto-syncing installed apps: $e');
      debugPrint('❌ [ChildPermissions] Stack trace: $stackTrace');
