...
class FlaggedCallsRemoteDataSource {
  ...
  Future<List<CallLogModel>> getFlaggedCalls({
    ...
    try {
      debugPrint('📥 [FlaggedCallsRemote] Fetching flagged calls...');
      debugPrint('   Path: parents/$parentId/children/$childId/flagged_calls');
      ...
      debugPrint('   Raw Firestore docs: ${snapshot.docs.length}');

      final flaggedCalls = snapshot.docs
          .map((doc) {
            try {
              final data = doc.data();
              return CallLogModel(
      ...
      debugPrint(
        '✅ [FlaggedCallsRemote] Loaded ${flaggedCalls.length} flagged calls from last 24h',
      );
      return flaggedCalls;
    } catch (e) {
      debugPrint('❌ [FlaggedCallsRemote] Error fetching flagged calls: $e');
      return [];
    }
  }
  ...
  Stream<List<CallLogModel>> getFlaggedCallsStream({
    ...
    debugPrint(
      '🔔 [FlaggedCallsRemote] Setting up stream for flagged calls...',
    );
    ...
    return firestore
        ...
        .map((snapshot) {
          debugPrint(
            '📥 [FlaggedCallsRemote] Stream update: ${snapshot.docs.length} flagged calls',
          );
  ...
  Future<int> getFlaggedCallsCount({
    ...
    try {
      ...
      debugPrint(
        '❌ [FlaggedCallsRemote] Error getting flagged calls count: $e',
      );
      return 0;
    }
  }
