...
class SuspiciousCallDetectorService {
  ...
  Future<void> scanCallLogs() async {
    debugPrint('🔍 [CallDetector] ========== SCANNING CALL LOGS ==========');
    debugPrint('   Child ID: $childId');
    debugPrint('   Parent ID: $parentId');
    debugPrint('   Scanning last 24 hours of calls...');
    debugPrint('==================================================');
  ...
  Future<bool> ensurePermissions() async {
    ...
    debugPrint('❌ [CallDetector] Permissions not granted');
    return false;
  }
  ...
  Future<Set<String>> loadContactNumbersNormalized() async {
    ...
    try {
      ...
      if (contactsStatus != PermissionStatus.granted) {
        debugPrint('⚠️ [CallDetector] Contacts permission not granted - requesting...');
        final requestResult = await Permission.contacts.request();
        if (requestResult != PermissionStatus.granted) {
          debugPrint('⚠️ [CallDetector] Contacts permission denied - will use call log name field only');
          return out;
        }
      }
      ...
      if (!hasPermission) {
        debugPrint('⚠️ [CallDetector] FlutterContacts permission denied');
        return out;
      }

      // Load all contacts
      debugPrint('📇 [CallDetector] Loading contacts from device...');
      final contacts = await FlutterContacts.getContacts(
        withProperties: true,
        withThumbnail: false,
      );

      debugPrint('📇 [CallDetector] Found ${contacts.length} contacts');
      ...
      debugPrint('✅ [CallDetector] Loaded ${out.length} normalized contact numbers');
      
      if (out.isEmpty) {
        debugPrint('ℹ️ [CallDetector] No contacts found - will rely on call log name field');
      }
    } catch (e) {
      debugPrint('⚠️ [CallDetector] Error loading contacts: $e');
      debugPrint('ℹ️ [CallDetector] Will use call log name field as fallback');
    }
    return out;
  }
  ...
  Future<void> runDetection() async {
    debugPrint('🔍 [CallDetector] ========== RUN DETECTION STARTED ==========');
    debugPrint('   Timestamp: ${DateTime.now().toIso8601String()}');
    
    // Check permissions
    final okPerm = await ensurePermissions();
    if (!okPerm) {
      debugPrint('❌ [CallDetector] Permissions not granted - cannot analyze call logs.');
      debugPrint('   ⚠️ Please grant CALL_LOG and CONTACTS permissions');
      return;
    }
    debugPrint('✅ [CallDetector] Permissions granted');
    ...
    debugPrint('📅 [CallDetector] Fetching calls from last 24 hours');
    debugPrint('   From: ${fromTime.toIso8601String()}');
    debugPrint('   To: ${now.toIso8601String()}');

    // Fetch calls and filter last 24 hours (optimized - early exit)
    try {
      final allCalls = await CallLog.get();
      debugPrint('📞 [CallDetector] Total calls in device: ${allCalls.length}');
      ...
      for (final call in sortedCalls) {
        ...
        if (ts >= fromTimestamp) {
          ...
          debugPrint('⏭️ [CallDetector] Reached 24-hour threshold - stopping early');
          debugPrint('   Processed ${recentCalls.length} recent calls, skipped ${sortedCalls.length - recentCalls.length} old calls');
          break;
        }
      }
      
      debugPrint('📞 [CallDetector] Calls from last 24 hours: ${recentCalls.length}');
      
      if (recentCalls.isEmpty) {
        debugPrint('😴 [CallDetector] No calls found in last 24 hours');
        return;
      }
      ...
      for (int i = 0; i < recentCalls.length && i < 5; i++) {
        ...
        final callTime = call.timestamp != null 
            ...
            : DateTime.now();
        debugPrint('   📞 Call ${i+1}: ${call.name ?? "Unknown"} (${call.number ?? "no number"}) - ${_getCallTypeString(call.callType)} at ${callTime.toIso8601String()}');
      }

      // Analyze all recent calls and apply rules
      debugPrint('🔍 [CallDetector] Starting analysis of ${recentCalls.length} calls...');
      await analyzeNewCalls(recentCalls);
      debugPrint('✅ [CallDetector] Analysis completed');
      
    } catch (e) {
      debugPrint('❌ [CallDetector] Error fetching call logs: $e');
    }
  }

  /// Analyze only NEW calls and create alerts
  Future<void> analyzeNewCalls(List<CallLogEntry> calls) async {
    debugPrint('🔍 [CallDetector] ========== ANALYZE NEW CALLS ==========');
    debugPrint('   Total calls to analyze: ${calls.length}');
    
    if (calls.isEmpty) {
      debugPrint('ℹ️ [CallDetector] No calls to analyze - exiting');
      return;
    }
    
    final contactNumbers = await loadContactNumbersNormalized();
    debugPrint('📇 [CallDetector] Loaded ${contactNumbers.length} contact numbers for comparison');
    ...
    try {
      ...
      debugPrint('⚠️ [CallDetector] Error loading watchlist: $e');
    }
    ...
    for (final kv in numberMap.entries) {
      ...
      if (!known) {
        debugPrint('🔍 [CallDetector] ⚠️ UNKNOWN NUMBER DETECTED: $number');
        debugPrint('   Name in call log: ${sampleCall.name ?? "null"}');
        debugPrint('   In contacts: $inContacts');
        debugPrint('   Call count: ${callsForNumber.length}');
      } else {
        debugPrint('✅ [CallDetector] Known number: $number (in contacts)');
      }
      ...
      if (watchlistNumbers.contains(number)) {
        ...
        if (!processedAlerts.contains(alertKey)) {
          ...
          debugPrint('🚨 [CallDetector] Flagged watchlist call: $number ($callType)');
        }
      ...
      if (!known) {
        debugPrint('🔍 [CallDetector] ⚠️ UNKNOWN NUMBER DETECTED: $number');
        debugPrint('   Total calls for this number: ${callsForNumber.length}');
        debugPrint('   Processing ALL calls for this unknown number...');
        ...
        for (final call in callsForNumber) {
          ...
          if (!processedAlerts.contains(alertKey)) {
            ...
            debugPrint('🚨 [CallDetector] ========== CREATING UNKNOWN CALL ALERT ==========');
            debugPrint('   Number: $number');
            debugPrint('   Call Type: $callType');
            debugPrint('   Duration: ${duration > 0 ? _formatDuration(duration) : "missed"}');
            debugPrint('   Timestamp: $timestamp');
            debugPrint('   Reason: $reason');
            debugPrint('   About to call createAlert()...');
            ...
            processedAlerts.add(alertKey);
            debugPrint('✅ [CallDetector] Alert created and saved for: $number');
            debugPrint('==================================================');
          } else {
            debugPrint('⏭️ [CallDetector] Alert already processed for: $number (timestamp: $timestamp)');
          }
        }
      } else {
        debugPrint('✅ [CallDetector] Known number: $number (in contacts) - skipping UNKNOWN_CALL rule');
      }
  ...
  Future<void> createAlert(
    ...
    debugPrint('🚨 [CallDetector] ========== CREATE ALERT CALLED ==========');
    debugPrint('   Number: $number');
    ...
    try {
      ...
      if (rule != 'UNKNOWN_CALL' && rule != 'WATCHLIST') {
        ...
        if (recentAlerts.docs.isNotEmpty) {
          debugPrint('⏭️ [CallDetector] Duplicate alert skipped for $number (rule: $rule)');
          return;
        }
      } else {
        ...
        if (callExample?.timestamp != null) {
          ...
          debugPrint('🔍 [CallDetector] Checking for duplicate alert: $number, rule: $rule, timestamp: ${callTimestamp.toIso8601String()}');
          ...
          for (final doc in recentAlerts.docs) {
            ...
            if (alertTimestamp != null) {
              ...
              if (timeDiff < 1000) {
                isDuplicate = true;
                debugPrint('⏭️ [CallDetector] Same call already flagged (timestamp diff: ${timeDiff}ms)');
                break;
              }
            }
          }
          ...
          debugPrint('✅ [CallDetector] No duplicate found - proceeding to create alert');
        }
      }
      ...
      debugPrint('📤 [CallDetector] Saving alert to Firebase: $alertId');
      debugPrint('   Path: parents/$parentId/children/$childId/flagged_calls/$alertId');
      debugPrint('   Number: $number');
      debugPrint('   Rule: $rule');
      debugPrint('   Reason: $reason');
      ...
      if (verifyDoc.exists) {
        debugPrint('✅ [CallDetector] Alert created and verified in Firebase: $reason');
      } else {
        debugPrint('❌ [CallDetector] ERROR: Alert was not saved to Firebase!');
      }
      ...
      try {
        ...
        debugPrint('✅ [CallDetector] Notification sent to parent');
      } catch (e) {
        debugPrint('⚠️ [CallDetector] Error sending notification: $e');
      }
    } catch (e) {
      debugPrint('❌ [CallDetector] Error creating alert: $e');
    }
  }
  ...
  Future<void> resetTimestamp() async {
    ...
    debugPrint('🔄 [CallDetector] Timestamp reset to 0');
  }
}
