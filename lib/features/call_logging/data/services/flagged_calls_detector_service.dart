...
class FlaggedCallsDetectorService {
  ...
  bool isSuspiciousCall({
    ...
    if (watchlist.contains(normalizedNumber)) {
      debugPrint('🚨 [FlaggedCallsDetector] Watchlist match: $number');
      return true;
    }
    ...
    if (_hasAlphabets(number)) {
      debugPrint(
        '🚨 [FlaggedCallsDetector] Invalid format (alphabets): $number',
      );
      return true;
    }
    ...
    if (approvedContacts.contains(normalizedNumber)) {
      debugPrint(
        '✅ [FlaggedCallsDetector] Known contact: $number - NOT suspicious',
      );
    ...
    debugPrint(
      '🚨 [FlaggedCallsDetector] Unknown number (not in contacts): $number',
    );
    ...
    if (hour >= 23 || hour < 6) {
      debugPrint(
        '🚨 [FlaggedCallsDetector] Odd hours call: $number at $hour:${timestamp.minute}',
      );
      return true;
    }
    ...
    try {
      ...
      if (snapshot.exists && snapshot.data() != null) {
        ...
        if (numbers.isNotEmpty) {
          final firebaseContacts = numbers
              .map((n) => _normalizeNumber(n.toString()))
              .toSet();
          debugPrint(
            '✅ [FlaggedCallsDetector] Loaded ${firebaseContacts.length} approved contacts from Firebase',
          );
      ...
      debugPrint(
        'ℹ️ [FlaggedCallsDetector] No Firebase approved contacts, loading device contacts...',
      );
      return await _loadDeviceContacts();
    } catch (e) {
      debugPrint(
        '⚠️ [FlaggedCallsDetector] Error loading approved contacts: $e',
      );
  ...
  Future<Set<String>> _loadDeviceContacts() async {
    try {
      ...
      if (!permission.isGranted) {
        debugPrint(
          '🔐 [FlaggedCallsDetector] Requesting contacts permission...',
        );
        final requestResult = await Permission.contacts.request();
        if (!requestResult.isGranted) {
          debugPrint('⚠️ [FlaggedCallsDetector] Contacts permission denied');
          return <String>{};
        }
      }

      debugPrint('📇 [FlaggedCallsDetector] Loading device contacts...');
      ...
      debugPrint(
        '✅ [FlaggedCallsDetector] Loaded ${numbers.length} contacts from device',
      );
      if (numbers.isNotEmpty) {
        debugPrint('   Sample contacts: ${numbers.take(3).join(', ')}...');
      }
      return numbers;
    } catch (e) {
      debugPrint('❌ [FlaggedCallsDetector] Error loading device contacts: $e');
      return <String>{};
    }
  }
  ...
  Future<Set<String>> loadWatchlist() async {
    try {
      ...
      debugPrint('⚠️ [FlaggedCallsDetector] Error loading watchlist: $e');
      return <String>{};
    }
  }
  ...
  Future<void> checkAndFlagCalls(List<CallLogModel> callLogs) async {
    try {
      debugPrint(
        '🔍 [FlaggedCallsDetector] Checking ${callLogs.length} calls for suspicious activity...',
      );
      ...
      debugPrint(
        '📇 [FlaggedCallsDetector] Loaded ${approvedContacts.length} approved contacts',
      );
      debugPrint(
        '👁️ [FlaggedCallsDetector] Loaded ${watchlist.length} watchlist numbers',
      );
      ...
      debugPrint(
        '✅ [FlaggedCallsDetector] Flagged $flaggedCount calls out of ${callLogs.length}',
      );
    } catch (e) {
      debugPrint('❌ [FlaggedCallsDetector] Error checking calls: $e');
    }
  }
  ...
  Future<void> _saveFlaggedCall(CallLogModel call) async {
    try {
      ...
      if (existing.docs.isNotEmpty) {
        debugPrint(
          '⏭️ [FlaggedCallsDetector] Call already flagged: ${call.number}',
        );
        return;
      }
      ...
      if (hour >= 23 || hour < 6) {
        reasons.add(
          'Odd hours (${call.dateTime.hour}:${call.dateTime.minute.toString().padLeft(2, '0')})',
        );
      }
      ...
      debugPrint(
        '✅ [FlaggedCallsDetector] Flagged call saved: ${call.number} - $reason',
      );
    } catch (e) {
      debugPrint('❌ [FlaggedCallsDetector] Error saving flagged call: $e');
    }
  }
  ...
  Future<void> _sendNotification(CallLogModel call) async {
    try {
      ...
      debugPrint('✅ [FlaggedCallsDetector] Notification sent to parent');
    } catch (e) {
      debugPrint('⚠️ [FlaggedCallsDetector] Error sending notification: $e');
    }
  }
}
