...
class _FlaggedCallsScreenState extends State<FlaggedCallsScreen> {
  final FlaggedCallsRemoteDataSource _dataSource = FlaggedCallsRemoteDataSource(
  ...
  void _startListening() {
    _streamSubscription = _dataSource
        ...
        .listen(
          ...
          onError: (error) {
            debugPrint('❌ [FlaggedCalls] Stream error: $error');
            if (mounted) {
  ...
  Future<void> _loadFlaggedCalls() async {
    ...
    try {
      debugPrint('🚨 [FlaggedCalls] Loading flagged calls...');
      ...
      debugPrint(
        '✅ [FlaggedCalls] Loaded ${_flaggedCalls.length} flagged calls',
      );
    } catch (e) {
      debugPrint('❌ [FlaggedCalls] Error: $e');
      if (!mounted) return;
      setState(() => _isLoading = false);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          '${widget.childName}\'s Flagged Calls',
                  ...
                  ? Center(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            'Suspicious calls will appear here',
                            style: TextStyle(
                              fontSize: mq.sp(0.04),
