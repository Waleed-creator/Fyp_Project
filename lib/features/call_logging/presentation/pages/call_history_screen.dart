...
class _CallHistoryScreenState extends State<CallHistoryScreen> {
  ...
  void _startListening() {
    debugPrint('🔔 [CallHistory] Starting real-time stream for call logs...');
    _streamSubscription = _dataSource
        .getCallLogsStream(parentId: widget.parentId, childId: widget.childId)
        .listen(
          (callLogs) {
            debugPrint(
              '📥 [CallHistory] Stream update: ${callLogs.length} calls',
            );

            // Filter last 24 hours
            final last24h = DateTime.now().subtract(const Duration(hours: 24));
            final recentLogs = callLogs
                .where((c) => c.dateTime.isAfter(last24h))
                .toList();
            ...
            final newCalls = recentLogs
                .where((call) => !_checkedCallIds.contains(call.id))
                .toList();
            if (newCalls.isNotEmpty) {
              debugPrint(
                '🆕 [CallHistory] ${newCalls.length} new calls detected - checking for suspicious activity...',
              );
          ...
          onError: (error) {
            debugPrint('❌ [CallHistory] Stream error: $error');
            if (mounted) {
  ...
  Future<void> _loadCallLogs() async {
    ...
    try {
      debugPrint('📞 [CallHistory] Loading call logs...');
      ...
      debugPrint(
        '✅ [CallHistory] Loaded ${_callLogs.length} call logs (last 24h)',
      );

      _checkFlaggedCalls(recentLogs);
    } catch (e) {
      debugPrint('❌ [CallHistory] Error: $e');
      if (!mounted) return;
      setState(() => _isLoading = false);
  ...
  List<CallLogModel> _dedupeAndSort(List<CallLogModel> logs) {
    ...
    for (final log in logs) {
      final key =
          '${log.id}_${log.number}_${log.type}_${log.dateTime.millisecondsSinceEpoch}_${log.duration}';
  ...
  Future<void> _checkFlaggedCalls(List<CallLogModel> callLogs) async {
    try {
      debugPrint(
        '🔍 [CallHistory] Checking ${callLogs.length} calls for suspicious activity...',
      );
      ...
      debugPrint('⚠️ [CallHistory] Error checking flagged calls: $e');
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(icon: const Icon(Icons.refresh), onPressed: _loadCallLogs),
        ],
      ),
      body: SafeArea(
        child: Column(
          children: [
            ...
            Container(
              ...
              child: SingleChildScrollView(
                ...
                child: Row(
                  children: ['All', 'Incoming', 'Outgoing', 'Missed'].map((
                    filter,
                  ) {
                    ...
                    return Padding(
                      ...
                      child: FilterChip(
                        ...
                        selectedColor: AppColors.darkCyan.withValues(
                          alpha: 0.2,
                        ),
              ...
              child: _isLoading
                  ? const Center(
                      child: CircularProgressIndicator(
                        valueColor: AlwaysStoppedAnimation(AppColors.darkCyan),
                      ),
                    )
