...
class ChildUrlTrackingService {
  final UrlTrackingFirebaseService _firebaseService =
  ...
  Future<void> uploadVisitedUrl({
    ...
    try {
      ...
      debugPrint('✅ Child URL uploaded to Firebase: $url');
    } catch (e) {
      debugPrint('❌ Error uploading child URL: $e');
  ...
  Future<void> batchUploadUrls({
    ...
    try {
      final visitedUrls = urls
          .map(
            (urlData) => VisitedUrlFirebase(
      ...
      debugPrint('✅ Child batch uploaded ${urls.length} URLs to Firebase');
    } catch (e) {
      debugPrint('❌ Error batch uploading child URLs: $e');
    }
  }
