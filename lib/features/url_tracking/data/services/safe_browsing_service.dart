...
class SafeBrowsingService {
  ...
  static const String _apiKey = 'AIzaSyCLCgFLHcyGngMoiihSFe9rI-9Qj_HjunQ';
  ...
  static Future<Map<String, dynamic>> checkUrlSafety(String url) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint('❌ Safe Browsing API error: ${response.statusCode}');
        return {
      ...
      debugPrint('❌ Safe Browsing API exception: $e');
      return {
  ...
  static Future<List<Map<String, dynamic>>> checkMultipleUrls(
    List<String> urls,
  ) async {
    try {
      final requestBody = {
      ...
      debugPrint('❌ Safe Browsing API exception: $e');
      return urls
          .map((url) => {'url': url, 'isSafe': true, 'error': 'Exception: $e'})
          .toList();
    }
  }
