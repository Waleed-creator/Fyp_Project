...
class UrlTrackingBloc extends Bloc<UrlTrackingEvent, UrlTrackingState> {
  ...
  UrlTrackingBloc({required UrlTrackingService urlTrackingService})
  ...
  Future<void> _onLoadVisitedUrls(
    ...
    try {
      ...
      debugPrint('📊 BLOC: Loaded ${allUrls.length} URLs from local storage');
      ...
      emit(
        UrlTrackingLoaded(visitedUrls: allUrls, hasPermissions: hasPermissions),
      );
    } catch (e) {
  ...
  Future<void> _onAddVisitedUrl(
    ...
    try {
      debugPrint('UrlTrackingBloc: Adding new URL: ${event.url.url}');
      ...
      if (state is UrlTrackingLoaded) {
        ...
        final exists = updatedUrls.any(
          (existingUrl) => existingUrl.url == event.url.url,
        );
        if (!exists) {
          updatedUrls.insert(0, event.url); // Add at the beginning
          debugPrint(
            'UrlTrackingBloc: Added new URL to list: ${event.url.url}',
          );

          // Update local storage
          try {
            await _urlTrackingService.localStorage.addVisitedUrl(event.url);
            debugPrint('💾 Bloc: URL saved to local storage: ${event.url.url}');
          } catch (e) {
            debugPrint('⚠️ Bloc: Error saving to storage: $e');
          }
        } else {
          debugPrint(
            'UrlTrackingBloc: URL already exists, skipping: ${event.url.url}',
          );
        }
        ...
        debugPrint('UrlTrackingBloc: Starting with new URL');
        emit(UrlTrackingLoaded(visitedUrls: [event.url], hasPermissions: true));

        // Save to storage
        try {
          await _urlTrackingService.localStorage.addVisitedUrl(event.url);
          debugPrint('💾 Bloc: Initial URL saved to storage: ${event.url.url}');
        } catch (e) {
          debugPrint('⚠️ Bloc: Error saving initial URL: $e');
        }
      }
    } catch (e) {
      debugPrint('UrlTrackingBloc: Error adding visited URL: $e');
  ...
  Future<void> _onStartUrlTracking(
    ...
    try {
      ...
      _urlStreamSubscription = _urlTrackingService.urlStream.listen((
        visitedUrl,
      ) {
        debugPrint(
          'UrlTrackingBloc: Processing URL from stream: ${visitedUrl.url}',
        );
  ...
  Future<bool> _checkUsageStatsPermission() async {
    ...
    try {
      ...
      final hasAccessibility = await _urlTrackingService
