...
import '../../../url_tracking/data/datasources/url_tracking_service.dart';
...
class _ChildPermissionScreenState extends State<ChildPermissionScreen> {
  ...
  final UrlTrackingService _realUrlService = UrlTrackingService();
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Permissions granted! Data collection started automatically.',
            ),
  ...
  Future<void> _startAutomaticDataCollection() async {
    try {
      ...
      if (currentUser == null) {
        debugPrint('❌ No user logged in for data collection');
        return;
      }

      debugPrint('🚀 Starting automatic data collection...');

      // Start real URL tracking service
      await _realUrlService.startTracking();
      debugPrint('✅ Real URL tracking service started');
      ...
      callLogDataSource.startContinuousMonitoring(
        parentId:
            currentUser.uid, // This should be actual parent ID in real app
        childId: currentUser.uid,
      );
      debugPrint('✅ Call logs monitoring started');
      ...
      debugPrint('✅ Automatic data collection started successfully!');

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              '🎉 Data collection started! Parent can now see your activity.',
            ),
      ...
      debugPrint('❌ Error starting automatic data collection: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildPermissionCard(
                    ...
                    isGranted:
                        false, // Accessibility service checked separately
                        ...
                        Expanded(
                          child: ElevatedButton.icon(
                            onPressed: () async {
                              ...
                              await _realUrlService.testAddUrl(
    ...
    return Card(
      ...
      child: ListTile(
        leading: CircleAvatar(
          ...
          child: Icon(isGranted ? Icons.check : icon, color: Colors.white),
        ),
        title: Text(
          title,
          style: TextStyle(
