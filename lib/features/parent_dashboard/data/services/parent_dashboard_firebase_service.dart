...
class ParentDashboardFirebaseService {
  ...
  Future<void> _updateTotalScreenTime({
    ...
    try {
      ...
      final totalMinutes = todayApps.fold<int>(
        0,
      ...
      debugPrint('❌ Error updating total screen time: $e');
    }
  }
  ...
  Stream<int> getTodayScreenTimeStream({
    ...
    return _screenTimeService
        .getDailyScreenTimeStream(
          childId: childId,
          parentId: parentId,
          date: todayStart,
        )
  ...
  Future<Map<String, dynamic>> getTodayAppUsageSummary({
    ...
    try {
      ...
      debugPrint('❌ Error getting today app usage summary: $e');
      return {
        'totalUsageMinutes': 0,
        'totalLaunches': 0,
        'totalApps': 0,
  ...
  Future<Map<String, dynamic>> getWeekAppUsageSummary({
    ...
    try {
      ...
      debugPrint('❌ Error getting week app usage summary: $e');
      return {
        'totalUsageMinutes': 0,
        'totalLaunches': 0,
        'totalApps': 0,
  ...
  Future<List<AppUsageFirebase>> getMostUsedApps({
    ...
    try {
      ...
      debugPrint('❌ Error getting most used apps: $e');
      return [];
    }
  }
  ...
  Future<void> deleteUrl({
    ...
    try {
      await _firestore
          ...
          .doc(urlId)
          .delete();
      debugPrint('✅ URL deleted successfully: $urlId');
    } catch (e) {
  ...
  Future<void> updateAppBlockStatus({
    ...
    try {
      ...
      await batch.commit();
      debugPrint('✅ App block status updated: $packageName -> $isBlocked');
    } catch (e) {
      debugPrint('❌ Error updating app block status: $e');
      rethrow;
    }
  }
  ...
  Stream<List<InstalledAppFirebase>> getInstalledAppsStream({
    ...
    debugPrint('   Path: parents/$parentId/children/$childId/installedApps');
    ...
    try {
      return _firestore
          ...
          .map((snapshot) {
            debugPrint(
              '📱 [ParentDashboardFirebaseService] Installed apps snapshot received: ${snapshot.docs.length} apps',
            );
            ...
            final apps = snapshot.docs
                .map((doc) {
                  try {
                    final data = doc.data();
                    debugPrint(
                      '   ✅ App: ${data['appName'] ?? 'Unknown'} (${data['packageName'] ?? 'Unknown'})',
                    );
                    return InstalledAppFirebase.fromJson(data);
                  } catch (e) {
                    debugPrint('❌ Error parsing app ${doc.id}: $e');
            ...
            debugPrint(
              '✅ [ParentDashboardFirebaseService] Parsed ${apps.length} installed apps',
            );
            return apps;
          })
          .handleError((error) {
            debugPrint(
              '❌ [ParentDashboardFirebaseService] Error in installed apps stream: $error',
            );
      ...
      debugPrint(
        '❌ [ParentDashboardFirebaseService] Error setting up installed apps stream: $e',
      );
