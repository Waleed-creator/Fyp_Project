...
class _ParentDashboardScreenState extends State<ParentDashboardScreen>
  ...
  final ParentDashboardFirebaseService _parentDashboardService =
  ...
  void initState() {
    ...
    context.read<ParentDashboardBloc>().add(
      LoadDashboardData(childId: widget.childId, parentId: widget.parentId),
    );
  }
      ...
      child: Column(
        ...
        children: [
          ...
          ...state.mostUsedApps
              ...
              .map(
                (app) =>
                    AppUsageCard(app: app, onTap: () => _showAppDetails(app)),
              ),
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          DailyScreenTimeChart(dailyData: state.dailyScreenTime),
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            SizedBox(height: 16),
            Text(
              'This will block URLs at the device level - they won\'t be accessible in any browser.',
            ),
            SizedBox(height: 16),
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              ScaffoldMessenger.of(
                context,
              ).showSnackBar(SnackBar(content: Text('VPN blocking started')));
            },
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              ScaffoldMessenger.of(
                context,
              ).showSnackBar(SnackBar(content: Text('VPN blocking stopped')));
            },
            style: ElevatedButton.styleFrom(
              backgroundColor: Colors.red,
              foregroundColor: Colors.white,
              ...
              if (minutes != null && minutes > 0) {
                try {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text(
                        'App limit set: ${app.appName} - $minutes minutes/day',
                      ),
                      backgroundColor: Colors.green,
                    ),
                  );
  ...
  Widget _buildInstalledAppsTab(ParentDashboardLoaded state) {
    ...
    return StreamBuilder<List<InstalledAppFirebase>>(
      stream: _parentDashboardService.getInstalledAppsStream(
        childId: widget.childId,
        parentId: widget.parentId,
      ),
      builder: (context, snapshot) {
        ...
        debugPrint(
          '📱 [ParentDashboardScreen] Installed apps count: ${installedApps.length}',
        );
        if (installedApps.isEmpty) {
          debugPrint(
            '⚠️ [ParentDashboardScreen] No installed apps found in Firebase',
          );
        ...
        final newApps = installedApps
            .where((app) => app.isNewInstallation)
            .toList();
        final userApps = installedApps
            .where((app) => !app.isSystemApp)
            .toList();
        final systemApps = installedApps
            .where((app) => app.isSystemApp)
            .toList();
    ...
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Column(
          ...
          children: [
            Text('Package: ${app.packageName}'),
            if (app.versionName != null) Text('Version: ${app.versionName}'),
            Text(
              ...
              if (minutes != null && minutes > 0) {
                try {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text(
                        'Global screen time limit set: $minutes minutes/day',
                      ),
                      backgroundColor: Colors.green,
                    ),
                  );
