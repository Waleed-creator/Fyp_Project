...
class ChildDeviceSyncService {
  ...
  final ScreenTimeFirebaseService _screenTimeService =
      ScreenTimeFirebaseService();
  final InstalledAppsFirebaseService _installedAppsService =
  ...
  void initialize({required String childId, required String parentId}) {
    ...
    _parentId = parentId;
    debugPrint(
      '✅ [ChildDeviceSyncService] Initialized for child: $childId, parent: $parentId',
    );
  }
  ...
  void startPeriodicSync() {
    if (_childId == null || _parentId == null) {
      debugPrint(
        '❌ [ChildDeviceSyncService] Not initialized. Call initialize() first.',
      );
      return;
    }

    if (_syncTimer != null) {
      debugPrint('⚠️ [ChildDeviceSyncService] Sync already running');
      return;
    }

    debugPrint('🔄 [ChildDeviceSyncService] Starting periodic sync...');
  ...
  void stopPeriodicSync() {
    _syncTimer?.cancel();
    _syncTimer = null;
    debugPrint('⏹️ [ChildDeviceSyncService] Stopped periodic sync');
  }
  ...
  Future<void> syncAllData() async {
    if (_isSyncing) {
      debugPrint(
        '⚠️ [ChildDeviceSyncService] Sync already in progress, skipping...',
      );
      return;
    }

    if (_childId == null || _parentId == null) {
      debugPrint('❌ [ChildDeviceSyncService] Not initialized');
      return;
    }
    ...
    try {
      debugPrint('🔄 [ChildDeviceSyncService] Starting full sync...');
      ...
      debugPrint(
        '⚠️ [ChildDeviceSyncService] Full sync requires individual method calls with data',
      );

      debugPrint('✅ [ChildDeviceSyncService] Full sync completed');
    } catch (e) {
      debugPrint('❌ [ChildDeviceSyncService] Error during sync: $e');
    } finally {
      _isSyncing = false;
    }
  }
  ...
  Future<void> syncAppUsage({
    ...
    try {
      ...
      debugPrint('✅ [ChildDeviceSyncService] App usage synced: $appName');
    } catch (e) {
      debugPrint('❌ [ChildDeviceSyncService] Error syncing app usage: $e');
    }
  }
  ...
  Future<void> syncAllAppUsage() async {
    ...
    debugPrint('📊 [ChildDeviceSyncService] Syncing all app usage...');
  }
  ...
  Future<void> syncScreenTime({
    ...
    try {
      ...
      debugPrint(
        '✅ [ChildDeviceSyncService] Screen time synced: $totalScreenTimeMinutes minutes',
      );
    } catch (e) {
      debugPrint('❌ [ChildDeviceSyncService] Error syncing screen time: $e');
    }
  }
  ...
  Future<void> syncInstalledApps({required List<InstalledApp> apps}) async {
    ...
    try {
      ...
      debugPrint(
        '✅ [ChildDeviceSyncService] Installed apps synced: ${apps.length} apps',
      );
    } catch (e) {
      debugPrint('❌ [ChildDeviceSyncService] Error syncing installed apps: $e');
    }
  }
