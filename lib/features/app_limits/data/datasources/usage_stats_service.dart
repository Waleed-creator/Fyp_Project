...
class UsageStatsService {
  ...
  Future<void> setGlobalRestriction(DateTime until) async {
    try {
      ...
      debugPrint('Error setting global restriction: $e');
    }
  }
  ...
  Future<void> clearGlobalRestriction() async {
    try {
      ...
      debugPrint('Error clearing global restriction: $e');
    }
  }
  ...
  Future<Duration> getWeeklyScreenTime() async {
    try {
      ...
      debugPrint('Error getting weekly screen time: $e');
      return Duration.zero;
    }
  }
  ...
  Future<AppUsageStats?> getAppUsageStatsForApp(String packageName, {
    ...
    try {
      ...
      debugPrint('Error getting app usage stats for $packageName: $e');
      return null;
    }
  }
  ...
  Future<int> getTodayUnlockCount() async {
    try {
      ...
      debugPrint('Error getting today unlock count: $e');
      return 0;
    }
  }
  ...
  Future<int> getWeeklyUnlockCount() async {
    try {
      ...
      debugPrint('Error getting weekly unlock count: $e');
      return 0;
    }
  }
  ...
  Future<String?> getCurrentForegroundApp() async {
    try {
      ...
      debugPrint('Error getting current foreground app: $e');
      return null;
    }
  }
  ...
  Future<void> startMonitoring() async {
    try {
      ...
      if (!hasPermission) {
        debugPrint('⚠️ [UsageStats] Usage Access permission not granted - real-time monitoring disabled');
        debugPrint('⚠️ [UsageStats] Please grant permission to enable real-time tracking');
        return;
      }
      ...
      debugPrint('✅ [UsageStats] Real-time monitoring started');
    } catch (e) {
      debugPrint('❌ [UsageStats] Error starting monitoring: $e');
    }
  }
  ...
  void _startRealTimeMonitoring() {
    if (_isRealTimeMonitoring) {
      debugPrint('⚠️ [UsageStats] Real-time monitoring already running');
      return;
    }
    
    _isRealTimeMonitoring = true;
    debugPrint('🔄 [UsageStats] Starting real-time foreground app monitoring...');
    ...
    _realTimeMonitoringTimer = Timer.periodic(const Duration(seconds: 2), (timer) async {
      try {
        ...
        if (currentApp != null && currentApp != _lastForegroundApp) {
          debugPrint('📱 [UsageStats] REAL-TIME: App changed to $currentApp');
          _lastForegroundApp = currentApp;
          
          // CRITICAL: Check app limit immediately when app changes
          debugPrint('🔒 [UsageStats] Checking app limit for: $currentApp');
          await checkAppRestrictionImmediately(currentApp);
          
          // You can add Firebase sync here for real-time updates to parent
          // await _syncCurrentAppToFirebase(currentApp);
        }
      } catch (e) {
        debugPrint('⚠️ [UsageStats] Error in real-time monitoring: $e');
      }
    });
    ...
    _realTimeSyncTimer = Timer.periodic(const Duration(seconds: 30), (timer) async {
      try {
        ...
        debugPrint('⚠️ [UsageStats] Error syncing to Firebase: $e');
      }
    });
    
    debugPrint('✅ [UsageStats] Real-time monitoring active (checking every 2s, syncing every 30s)');
  }
  ...
  Future<void> _syncUsageStatsToFirebase() async {
    try {
      ...
      debugPrint('📊 [UsageStats] Syncing ${stats.length} apps to Firebase (real-time update)');
      ...
      await firestore
        ...
        'totalScreenTime': stats.fold<int>(0, (total, s) => total + s.foregroundTime.inMinutes),
      ...
      debugPrint('✅ [UsageStats] Real-time data synced to Firebase');
      
    } catch (e) {
      debugPrint('❌ [UsageStats] Error syncing to Firebase: $e');
    }
  }
  ...
  void _startRestrictionChecker() {
    ...
    Timer.periodic(const Duration(seconds: 1), (timer) async {
      try {
        ...
        debugPrint('Error in restriction checker: $e');
      }
    });
  }
        ...
        if (limits.containsKey(stat.packageName)) {
          ...
          if (limit > 0 && stat.foregroundTime.inMinutes >= limit) {
            ...
            if (isRestricted != true) {
              ...
              debugPrint('🔒 App ${stat.packageName} restricted until tomorrow due to limit reached (${stat.foregroundTime.inMinutes}min >= ${limit}min)');
            ...
            try {
              ...
              if (currentApp == stat.packageName) {
                debugPrint('🚫 IMMEDIATELY CLOSING restricted app: $currentApp (${stat.foregroundTime.inMinutes}min >= ${limit}min)');
              ...
              debugPrint('Error closing restricted app: $e');
            }
          }
        }
      }
    ...
    try {
      ...
      if (limits.containsKey(packageName)) {
        ...
        if (limit > 0 && stat.foregroundTime.inMinutes >= limit) {
          ...
          debugPrint('🔒 IMMEDIATE RESTRICTION: App $packageName restricted (${stat.foregroundTime.inMinutes}min >= ${limit}min)');
          
          // Try to close the app immediately
          try {
            await requestLockNow();
            debugPrint('🚫 Sent lock command for restricted app: $packageName');
          } catch (e) {
      ...
      debugPrint('Error checking immediate restriction for $packageName: $e');
    }
  }
  ...
  Future<void> _notifyScreenTimeLimitReached(String packageName, int limit, int currentUsage) async {
    try {
      ...
      if (parentId == null || childId == null) {
        debugPrint('⚠️ [UsageStats] Parent/Child ID not found for notification');
        return;
      }
      ...
      debugPrint('✅ [UsageStats] Screen time limit notification sent to parent for ${appStat.appName}');
    } catch (e) {
      debugPrint('⚠️ [UsageStats] Error sending screen time limit notification: $e');
    }
  }
  ...
  Future<void> setAppRestriction(String packageName, DateTime until) async {
    try {
      ...
      debugPrint('Error setting app restriction: $e');
    }
  }
  ...
  Future<void> clearAppRestriction(String packageName) async {
    try {
      ...
      debugPrint('Error clearing app restriction: $e');
    }
  }
  ...
  Future<void> clearAllRestrictions() async {
    try {
      ...
      debugPrint('Error clearing all restrictions: $e');
    }
  }
  ...
  Future<void> stopMonitoring() async {
    try {
      ...
      debugPrint('⏹️ [UsageStats] Real-time monitoring stopped');
    } catch (e) {
      debugPrint('❌ [UsageStats] Error stopping monitoring: $e');
    }
  }
  ...
  Future<void> requestLockNow() async {
    try {
      ...
      debugPrint('Lock screen requested');
    } catch (e) {
      debugPrint('Error requesting lock screen: $e');
    }
  }
  ...
  Future<Map<String, dynamic>> getUsageReport() async {
    try {
      ...
      debugPrint('Error getting usage report: $e');
      return {};
    }
  }
}