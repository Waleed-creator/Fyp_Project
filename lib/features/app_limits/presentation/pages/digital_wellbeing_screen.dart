...
class _DigitalWellbeingScreenState extends State<DigitalWellbeingScreen> with TickerProviderStateMixin {
  ...
  Future<void> _checkPermissionAndLoadData() async {
    ...
    try {
      ...
      debugPrint('Error checking permission and loading data: $e');
    } finally {
      setState(() {
        _isLoading = false;
      });
  ...
  void _showSetLimitDialog(AppUsageStats app) async {
    ...
    showDialog(
      context: context,
      builder: (context) => StatefulBuilder(            // ← fixes DropdownButton rebuild
              ...
              Row(
                children: [
                  ...
                  DropdownButton<String>(
                    ...
                    onChanged: (v) {
                      if (v != null) setDialogState(() => unit = v); // ← use setDialogState
                    },
                  ),
                ],
              ),
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                if (!mounted) return;                                   // ← guard again after 2nd await
                navigator.pop();                                        // ← use captured navigator
                final msg = unit == 'hours'
                    ? 'Daily limit set: $amount hour(s)'
                    : 'Daily limit set: $minutes min';
                messenger.showSnackBar(SnackBar(content: Text(msg)));  // ← use captured messenger
              },
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                navigator.pop();                                        // ← use captured navigator
                final msg = unit == 'hours'
                    ? 'Global limit set: $amount hour(s)'
                    : 'Global limit set: $minutes min';
                messenger.showSnackBar(SnackBar(content: Text(msg)));  // ← use captured messenger
              },
              child: const Text('Save'),
            ),
